...
The project is defined as a full-stack TypeScript application where the frontend must follow entity["organization","Feature-Sliced Design","frontend architecture method"] (FSD) with strict import boundaries per layer/slice, and the backend — a modular monolith with orchestration and hexagonal architecture (ports & adapters). The correct "skill set" for an AI assistant in such a project is not "generate any files," but reproducible commands/intents that (a) scaffold module/slice structures according to conventions, (b) automatically maintain public APIs and boundaries, (c) add tests and migrations, (d) update CI/CD and deployment configs. FSD documentation explicitly describes layers and slices as code organization rules/conventions.

Key choices that are not fixed in the original requirements but are critical for "skills → templates → CI/CD":
- Test runners: in practice it makes sense to unify "BE+FE unit tests" on entity["organization","Vitest","vite-native test runner"] + entity["organization","Testing Library","user-centric ui testing"] (for FE) and Vitest (for BE). Vitest documents coverage (v8/istanbul) and integration with the Vite pipeline.
- HTTP framework on the backend is not specified: for hexagonal architecture a "thin" inbound adapter is convenient; entity["organization","Fastify","node http framework"] bets on a schema-based approach and recommends entity["organization","JSON Schema","schema specification"] for validation/serialization.
- Package manager/monorepo not specified: for full-stack TS it makes sense to use workspaces; entity["organization","pnpm","js package manager"] documents `pnpm-workspace.yaml` as the workspace root.
- Task orchestration (lint/test/build) in a monorepo: entity["organization","Turborepo","js build system"] documents `turbo.json`, caching, and remote caching, which directly increases the "automation potential" of skills (including in CI).
- Deploy on entity["company","Railway","deployment platform"]: the platform automatically uses a Dockerfile if one is found, and documents root directory settings for monorepos and start command.

Base Node version for infrastructure/CI: according to the official release list, the current Active LTS branch at the report date is v24 (while v25 is Current).

## Architectural Constraints and Target System Shape

Frontend: FSD defines 7 layers (app, processes, pages, widgets, features, entities, shared) and a semantic hierarchy of responsibilities; the Russian documentation separately fixes "layers" as responsibility levels and "slices" as grouping by domain/product value. Segments within slices in FSD separate code by purpose and have naming conventions for typical "purposes".

Backend: hexagonal architecture (ports & adapters) is originally described as a requirement to design the application so the core works independently of UI/DB, with specific integrations plugged in as adapters. A modular monolith in this context is a single deployment, but internally divided into modules with explicit interfaces and boundaries; modular monolith practices are described, in particular, as a set of architectural patterns/organizations supporting fast flow of changes.
...
Purpose: create a module (bounded context) in the modular monolith using hexagonal structure (domain/application/ports/adapters) and a public module API.
...
Purpose: add an application scenario (use-case) and an "orchestrator" (application service) that coordinates domain operations and ports. This corresponds to the idea of separating the core from adapters and concentrating "policies" in the application.
...
Purpose: create an inbound adapter (route/controller) with validation and request/response schemas. Fastify recommends a schema-based approach and JSON Schema for validation/serialization.
...
Purpose: declare an outbound port (repository interface) and implement an adapter on Prisma (including domain↔db mapping). Prisma positions Migrate as a generator of SQL migration history and a mechanism for synchronizing the Prisma schema and DB.
...
Purpose: add a model to `schema.prisma`, create a migration, and record commands for dev/prod. Prisma Migrate generates a history of `.sql` migrations, and for production recommends `migrate deploy`, explicitly stating that this is usually part of CI/CD.
...
Errors:
- `E_MIGRATION_CONFLICT` (modified migrations/history divergence) — Prisma warns about history checks on `migrate deploy`.
- `E_DB_CONNECTION`
...
Purpose: unit test scaffold for domain entities/policies and the application/use-case layer. Vitest supports v8/istanbul coverage and is configured through the `test` section.
...
Purpose: spin up a real DB in Docker during tests and run repositories/endpoints without mocks. Prisma explicitly describes the approach of "real DB in Docker" for integration tests. Testcontainers describes running throwaway services in Docker for tests, including a PostgreSQL module.
Inputs: `{ moduleName, suiteName, migrationCommand? }`
Outputs: setup (start/stop container), connection variables, minimal tests.
Preconditions: Docker engine available on machine/CI.
...
Purpose: a single runtime contract for DTOs (FE↔BE) with type inference: Zod describes validation schemas and returns a type-safe result after `parse`.
Inputs: `{ contractName, schemas {Input,Output,Error}? }`
Outputs: `packages/contracts` package, TS types, Zod schemas.
Preconditions: monorepo/workspaces configured.
...
Purpose: create a slice of the required layer according to FSD, including segments and a public API. Russian FSD documentation fixes the meaning of slices and business-oriented naming.
...
Purpose: a typical feature module: UI component, state/side-effects model, API client/hook; export through public API. Segment conventions in FSD are described in the tutorial (purpose-based folders).
...
Purpose: create a Zustand store (local for feature/slice or shared), selectors, and persist configuration. Zustand documents persist middleware as a way to save state to storage.
...
Purpose: standardize server-state: `useQuery/useMutation` hooks, keys, invalidation, typing. TanStack Query describes itself as "async state management / data fetching," and also documents the query key model and hooks.
...
Purpose: add a route module, link page, loaders/actions (if data routers are used). React Router documents `createBrowserRouter` as the recommended router for web projects and describes the trade-off: a full route tree upfront increases the initial bundle.
...
Purpose: unit tests for components and hooks, focusing on user behavior. Testing Library formulates the guiding principle "the more your tests resemble the way your software is used, the more confidence they can give you," and recommends avoiding testing implementation details.
...
Purpose: network mocking "at the request level" for UI unit/integration tests. entity["organization","Mock Service Worker","api mocking library"] describes itself as client-agnostic API mocking and directly provides a quick start for Vitest in Node tests.
...
Purpose: moving components/logic across FSD layers, respecting dependencies. FSD fixes the hierarchy and prohibition of "upward imports".
Inputs: `{ from, to, updateImports }`
Outputs: moved files, updated public APIs.
Preconditions: eslint rules for layers/public APIs enabled (INF‑03).
...
Purpose: reproducible linting based on flat config. ESLint documents flat config and migration, and TypeScript ESLint provides a quickstart for flat format with recommended rules.
...
Purpose: a unified formatter. Prettier documents configuration files (`.prettierrc`) and emphasizes the absence of a "global config" for consistency across teams. Tailwind supports an official prettier plugin that sorts classes in the recommended order.
Inputs: `{ printWidth, semi, singleQuote, tailwind: true }`
Outputs: `.prettierrc`, `.prettierignore`, plugin connection.
Preconditions: Prettier v3+.
...
Purpose: enforce architectural import boundaries. FSD provides an eslint-config for public API/layers-slices concepts. eslint-plugin-boundaries is designed to define element types and allowed connections.
...
Purpose: production images with build/runtime separation. Docker recommends multi-stage builds and minimal base images to reduce size and vulnerabilities.
...
Purpose: local dev stack (Postgres + backend), controlling startup order via `depends_on` and healthcheck. Docker Compose documents startup order and dependencies.
...
Purpose: CI on push/PR: install → lint → typecheck → test; for integration — Postgres service container. GitHub recommends `actions/setup-node` for consistent Node on runners and documents PostgreSQL service containers.
...
Purpose: deploy to Railway considering Dockerfile and env vars; Railway documents that with a Dockerfile present it will be used, and variables are available as environment variables on build/run; for monorepo — root directory.
Inputs: `{ serviceName, rootDir?, startCmd, envVars[] }`
Outputs: `railway.toml`/`railway.json` (optional), configuration instructions. Railway also documents Config-as-Code and Dockerfile priority. 
Preconditions: Dockerfile and start command ready. Railway separately explains the start command and env var expansion.
...
Purpose: describe tasks (build/lint/test/typecheck) and outputs for caching; Turborepo documents `turbo.json` and the concept of remote caching.
Inputs: `{ packages[], tasks[], outputsMap }`
Outputs: `turbo.json`, root package.json scripts.
Preconditions: workspace configured.
...
Vitest is positioned as a Vite-native test runner and can be used even without Vite, and also describes support for v8/istanbul coverage in the config. This is convenient for full-stack TS: unified commands and reporting.

**Alternative: Jest**
Jest remains widespread, but for TypeScript config `jest.config.ts` by default requires `ts-node` or a loader (Jest documentation explicitly describes this).

**Alternative: built-in node:test**
Node documents the built-in test runner (`node:test`) and separate guides on its usage. It can be useful for "minimal dependencies," but the ecosystem (mocks, jsdom-world) more often requires additional layers.

Comparison:

| Criterion | Vitest | Jest | node:test |
|---|---|---|---|
| TS-DX in monorepo | High (unified approach, Vite pipeline) | Good, but TypeScript configs more complex/varied | Basic, much added manually |
| Coverage | v8/istanbul in config | available, but setup depends on transformation | depends on surrounding tools |
...
Zustand is defined as the base "client state". At the same time, for server-state TanStack Query is usually simpler and cleaner, since it is oriented towards working with asynchronous sources and caching.

Comparison of popular options (as a reference table for the team):

| Library | Model | Strengths | Risks/Cons | When to choose |
|---|---|---|---|---|
| Zustand | Store + middleware | Simplicity and low boilerplate; middleware (persist etc.). | Easy to start storing "server-state" where a cache/invalidation is better | UI state, local caches, ephemeral state |
| entity["organization","Redux Toolkit","official redux toolset"] | Redux with opinionated defaults | `configureStore` gives good defaults. | More structure/ceremony than Zustand | Very large teams/strict patterns |
| entity["organization","Jotai","react atomic state"] | Atoms | Primitive/derived atoms. | Atom model doesn't suit everyone | Fine-grained state composition |
| entity["organization","Recoil","react state graph library"] | Atoms/selectors graph | Data graph atoms→selectors. | Dependency on project/ecosystem state | If graph model is specifically needed |

### ORM/DB Access

Prisma + PostgreSQL are fixed in the stack. Prisma Migrate describes the role of migrations and the history of `.sql` files. Therefore the recommendation: **Prisma as the primary ORM**, plus an "escape hatch" for complex SQL queries (raw SQL in Prisma or a dedicated query builder in read modules).

Comparison (for understanding trade-offs and possible evolution):

| Tool | Type | Key idea/doc reference | Pros | Cons | Fit for hex |
|---|---|---|---|---|---|
| Prisma | ORM + schema + migrate | Migrate: SQL migration history. | Type-safety, migrations, DX | Complex queries sometimes simpler outside ORM | Excellent via outbound adapter |
| entity["organization","TypeORM","typescript orm"] | ORM | Supports Active Record and Data Mapper. | Pattern flexibility, many DBs | More "magic"/variability | Good, but needs discipline |
| entity["organization","MikroORM","typescript data mapper orm"] | ORM (Data Mapper) | Identity Map + Unit of Work. | Strong DDD friendliness | Steeper learning curve, different philosophy | Excellent under DDD/hex |
| entity["organization","Drizzle ORM","typescript orm"] | Thin typed layer | Positions itself as a lightweight/typed layer over SQL. | SQL-thinking transparency | Fewer "batteries" than Prisma | Good as adapter/query layer |
| entity["organization","Kysely","type-safe query builder"] | Query builder | Type-safe SQL query builder. | Control and typing of complex SQL | No ORM model as such | Excellent as read adapter |

### Deployment Platforms: Railway and Alternatives

Railway is chosen. Important: Railway documents environment variables, environments, monorepo root directory, and that a Dockerfile will be used automatically if present.

Comparison (in case of compliance/cost/multi-region requirements):

| Platform | Monorepo | Prisma Migrations | Features |
|---|---|---|---|
| Railway | Root directory + start command. | Official Prisma→Railway guide. | Dockerfile auto-detect. Variables as env vars. |
| entity["company","Render","hosting platform"] | Yes (as PaaS) | Render describes pre-deploy command for Prisma migrations. | Strong emphasis on pre-deploy hooks |
| entity["company","Fly.io","app deployment platform"] | Yes | Prisma describes deployment on Fly.io and postinstall `prisma generate`. | Granular control, but more DevOps work |
| entity["company","Vercel","cloud platform"] | Usually yes | Backend guides and Postgres connection via Marketplace. | Great for FE/edge, API possible (Express guide). |
...
Reason: shared contracts/types (e.g., Zod contracts) and unified CI commands. pnpm documents workspaces and the fact that a workspace requires `pnpm-workspace.yaml` at the root. Railway documents monorepo deployment via root directory for each service.
...
Node: in CI and production it makes sense to fix **Node 24 LTS**, since the Active LTS branch at the report date is v24.

TypeScript: the base set of options must include `strict`, since TS describes `strict` as a general toggle for strict flags, including `strictNullChecks`. For large repositories, `incremental` is useful, which caches project graph information in `.tsbuildinfo`.
...
Fastify schema-based approach: if you describe a schema, Fastify validates/serializes and compiles schemas into performant functions. Prisma: for production migrations use `prisma migrate deploy` as part of CI/CD.
...
Prisma Migrate supports SQL migration history and schema synchronization.
...
`migrate deploy` is intended for staging/production (and is generally recommended as part of CI/CD).
...
Zod provides runtime validation and type inference from schema.

#### Unit Test Template (Vitest, BE)

Vitest documents configuration and coverage in `vitest.config.*`.
...
Prisma recommends the approach of using a real DB in Docker for integration tests. Testcontainers describes running a throwaway Postgres container.
...
Tailwind documents installation with Vite and emphasizes the "Vite plugin" as the most seamless integration path.

#### FSD Structure (example)

FSD layers and slices — the primary axis of organization.
...
Persist middleware is described in Zustand documentation as a way to store state in storage.
...
TanStack Query documents queries/mutations, the query model as a dependency on an async source, and `useQuery`.

#### Unit Test Template (RTL + Vitest)

Testing Library emphasizes the focus on user behavior and the harm of testing implementation details.
...
ESLint documents flat config and the structure of configuration files. TypeScript ESLint provides a quickstart specifically for flat config and recommended rules.
...
Prettier documents `.prettierrc` and emphasizes that there is no global configuration so that behavior is consistent when moving a project. Tailwind documents the official prettier plugin for class sorting.
...
- FSD eslint-config: rules `import-order`, `public-api`, `layers-slices`.
- eslint-plugin-boundaries: rule `boundaries/element-types` for describing element types and allowed connections.
- (optional, if using Nx): rule `@nx/enforce-module-boundaries` is documented as a way to prevent "unplanned cross-dependencies".
...
Docker documents Node.js containerization and best practices, including multi-stage builds.
...
Docker Compose: for controlled service startup, `depends_on` and healthcheck are used; Docker Compose directly describes startup order management. 
...
Railway documents:
- Dockerfile auto-detect and use of Dockerfile when present. 
- Variables as env vars (available for build/run and `railway run`). 
- Monorepo deployment via root directory and different start commands for services. 
- Start command and the nuance of env var expansion (for Dockerfile/image). 
- Config-as-code and Dockerfile priority. 

**Practical "skill flow" for deploying backend to Railway (minimum):**
1) Service Settings → Root Directory = `/apps/backend` (for monorepo). 
2) Variables: `DATABASE_URL`, `PORT`, `NODE_ENV=production`. 
3) Build uses Dockerfile automatically (if `Dockerfile` is in the root of the selected source directory). 
4) Prisma: apply migrations via `prisma migrate deploy` as part of the pipeline/start script (CI/CD preferred). 
5) If needed: configure environments (preview/staging/prod). 

### GitHub Actions CI (example)

GitHub documents `actions/setup-node` as the recommended way to use Node in Actions (consistent behavior), and also a separate guide on PostgreSQL service containers.  setup-node also supports built-in dependency caching. 
...
- Read and adopt FSD layer/slice rules (layers + slices).
- Learn to create a new FSD slice via FE‑01 and add a public API.
- Understand hexagonal structure: ports & adapters, core independence from UI/DB.
- Create module BE‑01 and use-case BE‑02, then endpoint BE‑03 (Fastify + schema).
- Master Prisma migrations (dev vs deploy) and the rule: in production — `migrate deploy` in CI/CD.
- Set up and run unit tests on Vitest, understand v8/istanbul coverage.
- Adopt the Testing Library philosophy (test behavior, not implementation details).
- Adopt formatting rules and Tailwind class sorting via Prettier plugin.
- Understand CI workflow (setup-node, Postgres service container) and be able to debug migration/test failures.
- Understand Railway: Variables, root directory for monorepo, Dockerfile detection.

### Recommended Resources for the Team (priority: official and localized where available)

- FSD Documentation: "Layers" and "Slices and Segments".
- Hexagonal architecture (original) and neutral description: Alistair Cockburn (hexagonal architecture) and AWS Prescriptive Guidance (ports/adapters, 2005).
- Prisma Migrate: overview, `migrate deploy`, and "development vs production".
- Vitest: configuration and coverage.
- Testing Library: introduction and guiding principles.
- Fastify: Validation & Serialization, TypeScript reference.
- ESLint flat config + migration guide; TypeScript ESLint quickstart.
- Prettier configuration and Tailwind class sorting plugin.
- Railway docs: builds/Dockerfiles, variables, monorepo deployment. 
- GitHub Actions: building & testing Node.js, PostgreSQL service containers.
- FSD overview (as an "entry text" for the team): FSD article on Habr and talks/links.
